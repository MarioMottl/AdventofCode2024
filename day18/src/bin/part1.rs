...
fn main() {
    ...
    for &(x, y) in byte_positions.iter().take(BYTES_TO_SIMULATE) {
        grid[y][x] = '#';
    }
    ...
    let shortest_path = bfs(&grid, (0, 0), (GRID_SIZE - 1, GRID_SIZE - 1));
    match shortest_path {
        Some(steps) => println!("Minimum number of steps: {}", steps),
